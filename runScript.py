...
while cap.isOpened():
    ...
    # Create a copy of the frame to draw on manually
    ...
    if results.boxes.id is not None:
        ...
        # --- NEW: Extract the probability score! ---
        confidences = results.boxes.conf.cpu().numpy() 

        # Add 'confidences' to our zip loop
        for box, track_id, class_id, conf in zip(boxes, track_ids, class_ids, confidences):
            ...
            # ==========================================
            # CUSTOM DRAWING (Boxes & Labels without IDs)
            # ==========================================
            # 1. Draw the bounding box and center dot
            cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
            cv2.circle(annotated_frame, current_center, 4, (0, 0, 255), -1)

            # 2. Create the label text (e.g., "Part 2 0.89")
            label = f"{class_name} {conf:.2f}"
            
            # 3. Draw a solid background box for the text so it's easy to read
            text_size, _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
            cv2.rectangle(annotated_frame, (x1, y1 - text_size[1] - 10), (x1 + text_size[0], y1), (0, 255, 0), -1)
            
            # 4. Draw the text inside the background box (black text on green background)
    ...
    # --- DISPLAY THE VIDEO ---
    cv2.imshow("YOLOv8 Custom Display", annotated_frame)
